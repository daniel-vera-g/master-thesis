...
% or \spl{key} for plural.
% Subsequent uses will show the short form.
...
\newacronym{sp}{SP}{System Prompt}
\newacronym{kb}{KB}{Knowledge Base}
\newacronym{aws-bedrock}{AWS Bedrock}{Amazon Bedrock}
\newacronym{aws-guardduty}{AWS GuardDuty}{Amazon GuardDuty}
\newacronym{boto3}{boto3}{AWS SDK for Python}

%----------------------------------------------------------------------------------------
% GLOSSARY TERMS
%----------------------------------------------------------------------------------------

\newglossaryentry{rego}{
    name={Rego},
    description={A high-level declarative language used by the Open Policy Agent (OPA) to express policies over complex data structures, such as JSON.}
}
...
\newglossaryentry{prompt-engineering}{
    ...
    description={The process of structuring text that can be interpreted and understood by a generative AI model. It is a concept in artificial intelligence, particularly natural language processing.}
}

\newglossaryentry{vector-database}{
    name=vector database,
    description={A type of database that stores data as high-dimensional vectors, which are mathematical representations of features or attributes. They are commonly used in machine learning for tasks like similarity search and retrieval-augmented generation (RAG).}
}

\newglossaryentry{facade-pattern}{
    name=Facade Pattern,
    description={A software design pattern that provides a simplified interface to a larger body of code, such as a complex library or framework. It hides the complexities of the larger system and provides a simpler API to the client.}
}

\newglossaryentry{command-pattern}{
    name=Command Pattern,
    description={A behavioral design pattern in which an object is used to encapsulate all information needed to perform an action or trigger an event at a later time. This decouples the object that invokes the operation from the one that knows how to perform it.}
}