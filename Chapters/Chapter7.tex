...
%----------------------------------------------------------------------------------------
\section{The Human-in-the-Loop in Action}
\label{sec:hitl_in_action}

A core component of the proposed framework is the Human-in-the-Loop (HITL) process for review and approval. This section details how human oversight is integrated into the automated workflow. The process begins when the system generates a security policy recommendation. This recommendation, along with its justification and the source IaC context, is presented to a security engineer or a designated reviewer through a user interface. The human expert then validates the correctness, relevance, and safety of the proposed policy. They have the authority to approve, reject, or modify the suggestion. This critical step ensures that automation accelerates the process without ceding final control, preventing the automatic application of flawed or insecure policies and providing a feedback mechanism for the system.
